...
func TestGetProductsRetorna200ComOsProdutos(t *testing.T) {
	...
	require.Equal(t, http.StatusCreated, post(t, server, "/product", `{"name":"Camiseta","price":30.99}`).Code)
	require.Equal(t, http.StatusCreated, post(t, server, "/product", `{"name":"Calca Jeans","price":89.99}`).Code)
